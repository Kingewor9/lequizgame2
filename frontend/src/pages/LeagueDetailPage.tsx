...
export const LeagueDetailPage: React.FC<Props> = ({ leagueId, onBack }) => {
  ...
  const handleRemoveMember = async (memberId: string, memberName: string) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setAlert({ message: `${memberName} removed from league`, type: 'success' });
      } else {
  ...
  const getInitials = (name: string) => {
    if (name === 'Me (The User)') return 'ME';
    return name.slice(0, 2).toUpperCase();
  };

  const isLeagueEnded = () => {
    if (!league) return false;
    return new Date() > new Date(league.end_date);
  };

  const progressPercent = league
    ...
    <div className="league-detail-page">
      ...
      <div className="league-detail-header">
        ...
        <div className="header-text">
          ...
          {league.description && (
            <p className="league-subtitle">{league.description}</p>
          )}
        </div>
      </div>
            ...
            {league.members.length === 0 ? (
              ...
              <div className="standings-list">
                {league.members
                  ...
                  .map((member) => {
                    const isMe = member.user_id === (user as any)?.id;
                    return (
                      <div
                        key={member.user_id}
                        className={`standing-row ${isMe ? 'standing-row--me' : ''}`}
                      >
